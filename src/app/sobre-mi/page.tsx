...
const posts: Post[] = [
  {
    ...
    date: '2025-03-14',
    summary:
      'Apuntes de cómo acabé mezclando escenas 3D con scroll y estados de React para contar historias sin aburrir.',
...
const events: Event[] = [
  ...
  },
  {
    year: '2025',
    title: 'Presente',
    description: `Este año por lo pronto he estado en Brasil, he empezado a aprender portugués, he hecho mucho monte y estoy montando un dúo de folk con un colega. Sigo leyendo, tocando la guitarra y entrenando al aire libre.`,
    tags: [
...
function deriveProjects(evs: Event[]) {
  const map = new Map<
    string,
    { label: string; kind: 'proyecto' | 'trabajo' | 'voluntario'; years: number[] }
  >();
  evs.forEach((e) => {
    e.tags.forEach((t) => {
...
function collectThemes(evs: Event[]) {
  ...
  evs.forEach((e) => {
    e.tags.forEach((t) => {
      const type = normalizaTipo(t.type);
      if (type === 'estudios' || t.label.toLowerCase().includes('ingenier'))
        themes['Ingeniería'].push(t.label);
      if (type === 'musica') themes['Música'].push(t.label);
      if (type === 'hobby' && t.label.toLowerCase().includes('mont'))
        themes['Montaña'].push(t.label);
...
function Stat({ value, label }: { value: number | string; label: string }) {
  return (
    <div className={styles.timelineCard} style={{ textAlign: 'center' }}>
      ...
      <div style={{ marginTop: 6, fontSize: '0.9rem', color: 'var(--color-font-secondary)' }}>
        {label}
      </div>
    </div>
  );
...
function PostCard({ p }: { p: Post }) {
  return (
    ...
    >
      <div style={{ fontSize: '0.85rem', color: 'var(--color-font-tertiary)', marginBottom: 6 }}>
        {formatDate(p.date)}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {p.tags.map((t) => (
          <span key={t} className={`${styles.tag} ${styles['tag-proyecto']}`}>
            {t}
          </span>
        ))}
      </div>
        ...
        <div id="content" className={styles.content} style={{ pointerEvents: 'auto' }}>
          ...
          <div className="container" style={{ padding: '6rem 0 2rem' }}>
            <div className={`mw-48 ${styles.introText}`}>
              ...
              <p className="mb-8">
                No te estoy vendiendo nada. Si algo de aquí te sirve, ya habrá tiempo de hablar.
                Mientras tanto, <span className="highlight-2">blog</span>,{' '}
                <span className="highlight-2">proyectos</span> y alguna que otra{' '}
                <span className="highlight-2">idea a medio hacer</span>.
              </p>
            </div>
          </div>
                ...
                <div key={name} className={styles.timelineCard} style={{ flex: '1 1 280px' }}>
                  ...
                  <ul className="label-list" style={{ marginBottom: 0 }}>
                    {Array.from(new Set(items))
                      .slice(0, 12)
                      .map((x) => (
                        <li key={x} className="label-list-item">
                          {x}
                        </li>
                      ))}
                  </ul>
          ...
          <Section id="contacto" title="Si te apetece charlar" kicker="Puertas abiertas">
            ...
            <p className="mb-1" style={{ color: 'var(--color-font-secondary)', marginTop: 16 }}>
              *Si algún día ofrezco algo más concreto, lo pondré aquí. De momento, a disfrutar del
              proceso.
            </p>
          </Section>
        </div>
