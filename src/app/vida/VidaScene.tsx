...
import { fetchVidaCategorias, fetchVidaItems, fetchVidaLibrosPreview, VidaCategoria, VidaItem, VidaLibrosPreview } from "../utils/vidaApi";
...
const buildDetalle = (
  ...
  const relacionados = (item.relacionados || [])
    ...
    .map((r) => ({ slug: r.slug, titulo: r.titulo, categoria: catMap[r.categoria]?.nombre || r.categoria }));
...
const VidaScene = () => {
  ...
  useEffect(() => {
    let vivo = true;
    (async () => {
      const [cats, its, preview] = await Promise.all([fetchVidaCategorias(), fetchVidaItems(), fetchVidaLibrosPreview()]);
      if (!vivo) return;
      if (!cats.length) {
        setDataError(true);
        return;
  ...
  useEffect(() => {
    ...
    const engine = new VidaEngine(canvasRef.current, miniRef.current, categorias, items, librosDestacados, tema, true, {
      onHover: setHover,
      onCursorMove: (x, y) => {
        if (tipRef.current) tipRef.current.style.transform = `translate(${x + 16}px, ${y + 14}px)`;
      },
      onSelectItem: (item) => {
    ...
    return (
      <div className={styles.root}>
        <div className={styles.loading}>
          <div className={styles.loadingLabel}>No se pudo cargar el archipiélago. Inténtalo de nuevo más tarde.</div>
        </div>
      </div>
    );
  }
    ...
    <div className={styles.root}>
      ...
      <div className={styles.overlay}>
        <div className={styles.intro}>
          ...
          <div className={styles.subtitle}>Cada isla es una categoría, no un año. Arrastra el mar para descubrirlas.</div>
        ...
        {showLecturas && librosPreview && <VidaLecturasPanel lecturas={librosPreview} onClose={() => setShowLecturas(false)} />}
